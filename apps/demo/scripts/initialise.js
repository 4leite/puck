...
(async () => {
  ...
  await db.execute(
    `CREATE TABLE IF NOT EXISTS pages ( key TEXT PRIMARY KEY, data JSON ) WITHOUT ROWID;`
  );

  for (const key of Object.keys(initialData)) {
    const data = JSON.stringify(initialData[key]);
    db.execute({
      sql: `INSERT OR IGNORE INTO pages ( key, data ) VALUES( $key, $data )`,
      args: [key, data],
    });
  }
})();
